...
export default class LifeMatrix{
    ...
    private getNumber(i: number, j: number): number{
        const neighbours: (number|undefined)[] = [...this.rowNeighbours(i-1, j),
                                                 ...this.rowNeighbours(i+1, j),
                                                 this._numbers[i][j-1], this._numbers[i][j+1]];
        const nLives = neighbours.reduce((res: number, cur) => {return cur? res + 1 : res}, 0);
...
function fromAlive(nLives: number ): number{
    return +(nLives===2 || nLives===3); // live cell stay live if it has 2 or 3 neighpurs only
}
function fromDead (nlives: number): number{
    return +(nlives===3); // dead cell become alive if it has 3 neighbours exact
} 